...
def get_hot3d_front_camera():
    R_x = get_rotmat_x(-np.pi/2)
    R_y = get_rotmat_y(np.pi)
    R = np.matmul(R_y, R_x)
    R = torch.from_numpy(R).unsqueeze(0)
    T = torch.FloatTensor([0., -2.0, 2.0]).unsqueeze(0)
    # T = torch.FloatTensor([0., -1.3, 1.5]).unsqueeze(0)
    return R, T

def get_hot3d_side_camera():
    R_x = get_rotmat_x(-np.pi/2)
    R_y = get_rotmat_y(np.pi)
    R_z = get_rotmat_z(np.pi/2)
    R = np.matmul(R_y, R_x)
...
class Renderer():
    def __init__(self, device, img_size=512, camera=None, fps=30):
        ...
        if camera == "h2o_front":
            ...
            R, T = get_hot3d_front_camera()
        elif camera == "hot3d_side":
            lights_front = PointLights(
                device=device, 
                location=[[0.0, -1, 0.5]], 
                ambient_color=((0.6, 0.6, 0.6),), 
        ...
        self.colors = {
            ...
            "obj0": [235/255, 206/255, 135/255],
            "obj1": [235/255, 206/255, 135/255],
            "obj2": [235/255, 206/255, 135/255],
            "obj3": [235/255, 206/255, 135/255],
            "obj4": [235/255, 206/255, 135/255],
            "obj5": [235/255, 206/255, 135/255],
        }
