...
def pc_normalize(pc, return_params=False):
    ...
    if scale == 0: scale = np.ones_like(scale)
    pc = pc / scale
    if return_params:
...
def proc_cond_contact_estimator(obj_scale, obj_feat, enc_text, nobjs, npts, use_scale):
    if use_scale:
...
def proc_obj_feat_cov(
    ...
    obj_feat_global = obj_feat[:, :, 0, :1024]  # [1, 6, 1024]
    if use_contact_feat:
        condition = proc_cond_contact_estimator(obj_scale, obj_feat, enc_text, nobjs, npts, use_scale)
        est_contact_map = contact_estimator.decode(condition.flatten(0,1)).reshape(-1, nobjs, npts, 1)
...
def proc_obj_feat_final(
    ...
    obj_feat_cov, est_contact_map, est_contact_map_plot = proc_obj_feat_cov(
        contact_estimator, 
        obj_scale, obj_feat, enc_text, nobjs, npts, 
        use_scale, use_contact_feat, 
    )
    if use_obj_scale_centroid:
        obj_scale_expand1 = obj_scale.unsqueeze(-1)
        obj_feat_final = torch.cat([obj_feat_cov, obj_scale_expand1, obj_cent.unsqueeze(0)], dim=-1)
    else:
...
def get_hand2obj_dist(hand_joints, obj_pc, obj_pc_normal, valid_mask_hand, valid_mask_obj):
    B, T, J, _ = hand_joints.shape
    _B, _T, N, P, _ = obj_pc.shape
    assert B==_B and T==_T
    # make deviation for invalid objects, to preserve from selecting them as the contact object
    _mask = valid_mask_obj.reshape(B, T, N, 1, 1).expand_as(obj_pc)
    obj_pc[_mask] = 100 + obj_pc[_mask].abs()
    ...
    hand_nn_idx_expand = hand_nn_idx.unsqueeze(-1).expand_as(hand_joints)
    ...
    hand_nn_dist_obj_idx = hand_nn_idx.reshape(B, T, -1) // 1024 # the point belongs to which object
    return hand_dist_values_xyz, hand_nn_dist, obj_pc_contact, hand_nn_dist_obj_idx
...
def proc_refiner_input(
    ...
    threshold = 0.02
    bs, T, N = pred_obj.shape[:3]
    # drop invalid frames (where all objects and hand are unavailable)
    # valid_frame_left = torch.logical_and(valid_mask_obj.sum(-1)>0, valid_mask_lhand)
    # valid_frame_right = torch.logical_and(valid_mask_obj.sum(-1)>0, valid_mask_rhand)
    ...
    lhand_dist_values_xyz, lhand_dist_values, obj_pc_contact_lhand, lhand_nn_dist_obj_idx \
        = get_hand2obj_dist(lhand_joints, tf_obj_pc, tf_obj_pc_normal, valid_mask_lhand, valid_mask_obj)
    rhand_dist_values_xyz, rhand_dist_values, obj_pc_contact_rhand, rhand_nn_dist_obj_idx \
        = get_hand2obj_dist(rhand_joints, tf_obj_pc, tf_obj_pc_normal, valid_mask_rhand, valid_mask_obj)
    
    lhand_attn = torch.exp(-50*lhand_dist_values_xyz)
    rhand_attn = torch.exp(-50*rhand_dist_values_xyz)
    
    cov_map = cov_map.unsqueeze(1)
    cov_map = cov_map.expand(-1, T, -1, -1)
    input_lhand = torch.cat([pred_lhand, lhand_joints.reshape(bs, T, -1), lhand_attn.reshape(bs, T, -1), tf_obj_pc.reshape(bs, T, -1, 3).norm(dim=-1), cov_map.flatten(2)], dim=-1)
    input_rhand = torch.cat([pred_rhand, rhand_joints.reshape(bs, T, -1), rhand_attn.reshape(bs, T, -1), tf_obj_pc.reshape(bs, T, -1, 3).norm(dim=-1), cov_map.flatten(2)], dim=-1)
    if return_psuedo_gt:
        lhand_obj_mask = torch.gather(valid_mask_obj, dim=-1, index=lhand_nn_dist_obj_idx)
        rhand_obj_mask = torch.gather(valid_mask_obj, dim=-1, index=rhand_nn_dist_obj_idx)
        lhand_contact_joint_mask = (lhand_dist_values < threshold)*torch.logical_and(lhand_obj_mask, valid_mask_lhand.unsqueeze(-1))
        rhand_contact_joint_mask = (rhand_dist_values < threshold)*torch.logical_and(rhand_obj_mask, valid_mask_rhand.unsqueeze(-1))
