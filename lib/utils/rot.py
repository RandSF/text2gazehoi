...
def quaternion_to_rotation_matrix(quat):
    ...
    """
    norm_quat = quat
    norms = torch.linalg.norm(norm_quat, ord=2, dim=1, keepdim=True)
    norms = torch.where(torch.isnan(norms), norms, 1)   # it is okay to add a small eps isntead of doing so
    norm_quat = norm_quat / norms
