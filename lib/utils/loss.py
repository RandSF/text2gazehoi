...
def get_relative_orientation_loss(
    ...
    pred_ro_lhand = get_ro(pred_lhand, pred_obj, mask_lhand, mask_obj)
    targ_ro_lhand = get_ro(targ_lhand, targ_obj, mask_lhand, mask_obj)
    pred_ro_rhand = get_ro(pred_rhand, pred_obj, mask_rhand, mask_obj)
    targ_ro_rhand = get_ro(targ_rhand, targ_obj, mask_rhand, mask_obj)

    if len(pred_ro_lhand) == len(targ_ro_lhand) == 0:   # no object is valid
        ro_lhand_loss = torch.zeros(1)[0].to(pred_ro_lhand)
    else:
        ro_lhand_loss = F.mse_loss(pred_ro_lhand, targ_ro_lhand)
    if len(pred_ro_rhand) == len(targ_ro_rhand) == 0:   # no object is valid
        ro_rhand_loss = torch.zeros(1)[0].to(pred_ro_lhand)
    else:
...
def get_ro(hand, obj, hand_valid_mask, object_valid_mask):
    # you can easily vertify that `object_valid_mask` is a subset of `hand_valid_mask` by running
    # torch.all(hand_valid_mask.unsqueeze(-1).expand_as(object_valid_mask) * object_valid_mask == object_valid_mask)

    hand_orient = hand.unsqueeze(2).expand(-1, -1, 6, -1)[object_valid_mask][..., 3:9]
    obj_orient = obj[object_valid_mask][..., 3:9]
    ...
    if torch.isnan(hand_orient_rotmat).any() or torch.isnan(obj_orient_rotmat).any() or torch.isnan(ro_hand_obj).any():
        print("gugugaga!")
    return ro_hand_obj
...
def get_filtered_loss_valid_mask(loss, valid_mask, loss_weight=None):
    ...
    if filtered_loss.ndim == 2:
        filtered_loss_summed = filtered_loss.sum(-1)
        valid_mask_summed = valid_mask.sum(-1)
    elif filtered_loss.ndim == 3:
        filtered_loss_summed = filtered_loss.sum([-1, -2])
        valid_mask_summed = valid_mask.sum([-1, -2])
...
def get_filtered_loss_valid_map(loss, valid_map, weight=None):
    ...
    filtered_loss_summed = filtered_loss.sum([1, 2, 3, 4]) # batch, nframes, 6, 1024, 21
    valid_map_summed = valid_map.sum([1, 2, 3, 4])
...
def get_penetration_loss(
    ...
    batch_size, nobjs, npts = obj_pc_org.shape[:3]
    transf_obj_pc = get_transformed_obj_pc(pred_X0_obj, obj_pc_org, dataset_name, obj_pc_top_idx)
    transf_obj_pc = transf_obj_pc.reshape(-1, nobjs*npts, 3)
