...
def get_object_hand_info(
    ...
    # fix objects
    obj_names = ['holder_gray', 'food_waffles', 'puzzle_toy', 'aria_small', 'bottle_ranch', 'dino_toy']
    is_lhand_list = [1, ]
    is_rhand_list = [1, ]
    obj_pc_list = []
    normalized_obj_pc_list = []
    ...
    obj_file = [get_obj_file(obj_root, obj_n, data_config) for obj_n in obj_names]
    obj_mesh = [trimesh.load(fp, maintain_order=True) for fp in obj_file]
    # obj_mesh = trimesh.exchange.load.load_mesh(obj_file, process=False)
    obj_verts_org = [proc_torch_cuda(mesh.vertices.copy()) for mesh in obj_mesh]
    for obj_name in obj_names:
        point_set, obj_pc, obj_pc_normal, _ = object_model(obj_name)
        normalized_obj_pc, obj_norm_cent, obj_norm_scale = pc_normalize(obj_pc, return_params=True)
        obj_pc = proc_torch_cuda(obj_pc).unsqueeze(0)
        obj_pc_normal = proc_torch_cuda(obj_pc_normal).unsqueeze(0)
        normalized_obj_pc = proc_torch_cuda(normalized_obj_pc).unsqueeze(0)
        ...
        obj_pc_normal_list.append(obj_pc_normal)
        normalized_obj_pc_list.append(normalized_obj_pc)
        point_set_list.append(torch.from_numpy(point_set))
        obj_cent_list.append(obj_norm_cent)
        obj_scale_list.append(obj_norm_scale)
    obj_verts_list = torch.stack(obj_verts_org).unsqueeze(0)
    obj_faces_list = [[torch.from_numpy(mesh.faces).cuda() for mesh in obj_mesh],]

    is_lhand_list = proc_long_torch_cuda(is_lhand_list).unsqueeze(0)
    is_rhand_list = proc_long_torch_cuda(is_rhand_list).unsqueeze(0)
    obj_pc_list = torch.stack(obj_pc_list, dim=1)
    obj_pc_normal_list = torch.stack(obj_pc_normal_list, dim=1)
    normalized_obj_pc_list = torch.stack(normalized_obj_pc_list, dim=1)
    point_set_list = proc_long_torch_cuda(torch.stack(point_set_list, dim=1))
    obj_pc_top_idx_list = proc_long_torch_cuda(obj_pc_top_idx_list)
    obj_cent_list = torch.stack(obj_cent_list)
    obj_scale_list = proc_torch_cuda(obj_scale_list).unsqueeze(0)
    return (
        is_lhand_list, is_rhand_list, 
        obj_pc_list, obj_pc_normal_list, 
        normalized_obj_pc_list, point_set_list, 
...
def get_valid_mask_bunch(is_lhand, is_rhand, max_nframes, num_object, duration):
    ...
    for idx in range(len(duration)):
        valid_mask_lhand, valid_mask_rhand, valid_mask_obj \
            = get_valid_mask(is_lhand[idx], is_rhand[idx], max_nframes, 6, duration[idx])
