...
class TextHOM(nn.Module):
    def __init__(self, hand_nfeats=99, obj_nfeats=9, latent_dim=512, 
        ...
        self.seq_obj_gate = nn.Sequential(
            nn.SiLU(self.latent_dim),
            nn.Linear(self.latent_dim, 1)
        )
        self.seq_obj_adaln = nn.Linear(self.latent_dim, self.latent_dim*2)
        ...
        self.out_fc_obj = OutFC(self.input_feats_obj, self.latent_dim*2)
    ...
    def forward(
        ...
        feat_obj = self.init_fc_obj(x_obj.flatten(1, 2)).reshape(nframe, n_obj, bs, -1)
        mixed_obj = torch.sum(nn.functional.softmax(self.seq_obj_gate(feat_obj), dim=-1) * feat_obj, dim=1)
        x_init_obj_gamma, x_init_obj_beta = torch.chunk(self.seq_obj_adaln(mixed_obj), 2, -1)  # [Nf, Bs, D*2] -> [Nf, Bs, D]
        x_init_obj = mixed_obj * x_init_obj_gamma + x_init_obj_beta  # [Nf, Bs, D]

        # import pdb
        # pdb.set_trace()
        x_init = torch.stack((x_init_lhand, x_init_rhand, x_init_obj), dim=1)   # [Nf, Bs, D], [Nf, Bs, D] -> [Nf, 3, Bs, D]
        ...
        xseq = torch.cat((emb, xseq), dim=0)    # [1+Nf*3, Bs, D]

        if valid_mask_obj is not None:
            bs = x_obj.shape[0]
            emb_mask = torch.ones((bs, 1), device=x_obj.device, dtype=bool)
            aug_mask_no_emb = torch.stack(
                [valid_mask_lhand, valid_mask_rhand, torch.any(valid_mask_obj, dim=-1)], dim=-1
            ).reshape(bs, -1)
        ...
        x_enc = x_enc.reshape(nframe, 2+1, bs, -1)
        x_enc_lhand = x_enc[:, 0]
        x_enc_rhand = x_enc[:, 1]
        x_enc_obj = x_enc[:, 2:]

        x_obj_pred = torch.cat([feat_obj, x_enc_obj.expand_as(feat_obj)], dim=-1)  # [Nf, No, Bs, D*2]
        pred_obj_offset = self.out_fc_obj(x_obj_pred.flatten(0, 1)).reshape(bs, nframe, n_obj, -1)
        if active_flag is not None:
            ### only update the active objects  active_flag: [Bs, Nf, No]
            active_flag_extend = active_flag.unsqueeze(-1)   # [Bs, Nf, No, 1]
            pred_obj = x_obj + pred_obj_offset*active_flag_extend
        else:
            pred_obj = x_obj + pred_obj_offset
...
class HandObjectEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=int(300*3)):
