...
class Diffusion(nn.Module):
    ...
    def forward(
        ...
        if timesteps == None:
            ...
            x_tilde_obj = torch.sqrt(used_alpha_bars[...,None]) * x_obj + torch.sqrt(1 - used_alpha_bars[...,None]) * epsilon_obj
        ...
        ### fill the inactive objects
        # active_flag = active_flag.unsqueeze(1).unsqueeze(-1).expand_as(x_obj).bool() # [Bs, Nf(1), No, D(1)]
        # pred_X0_obj = torch.where(active_flag, pred_X0_obj, x_obj)
    ...
    def sampling(
        ...
        sample_obj = torch.randn([sampling_number, max_nframes, 6, obj_nfeats]).to(device)
