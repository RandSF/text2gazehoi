...
from lib.utils.hot3d_utils import (
    hot3d_proc_obj_feat_final, 
    hot3d_proc_obj_feat_final_train
)
...
def main(config):
    ...
    texthom = torch.nn.parallel.DistributedDataParallel(texthom)
    ...
    with tqdm.tqdm(range(nepoch)) as pbar:
        for epoch in pbar:
            ...
            optimizer.zero_grad()
            for s, item in enumerate(dataloader):
                ...
                init_frames = item['init_frame'].cuda()
                ...
                active_flag = item['active_flag'].cuda()
                ...
                losses = lambda_simple*simple_loss \
                       ...
                       + lambda_ro*ro_loss
                (losses/config.accumulate_step).backward()
                # for tensor in [pred_X0_lhand, pred_X0_rhand,pred_X0_obj, simple_loss, dist_map_loss, ro_loss]:
                #     if torch.isnan(tensor).any():
                #         print(f"::{item['idx_orig'].item()}::")
                #         print([torch.isnan(x).any().item() for x in [pred_X0_lhand, pred_X0_rhand, pred_X0_obj, simple_loss, dist_map_loss, ro_loss]])
                #         print([torch.isnan(x).all().item() for x in [pred_X0_lhand, pred_X0_rhand, pred_X0_obj, simple_loss, dist_map_loss, ro_loss]])
                #         print(text)
                #         print(item['text_orig'])
                #         print("="*22)
                #         break
                if (s+1) % config.accumulate_step == 0:
                    optimizer.step()
                    optimizer.zero_grad()
            ...
            if s% config.accumulate_step != 0:  # last gradient hasnot been updated
                optimizer.step()
